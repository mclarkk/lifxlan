...
def unpack_lifx_message(packed_message):
	...
	if message_type == MSG_IDS[GetService]:
		...
		message = StateService(target_addr, source_id, seq_num, service, port, ack_requested, response_requested)
		...
		message = StateHostInfo(target_addr, source_id, seq_num, signal, tx, rx, reserved1, ack_requested, response_requested)
		...
		message = StateHostFirmware(target_addr, source_id, seq_num, build, reserved1, version, ack_requested, response_requested)
		...
		rx = struct.unpack("I", payload_str[8:12])[0]
		reserved1 = struct.unpack("h", payload_str[12:14])[0]
		message = StateWifiInfo(target_addr, source_id, seq_num, signal, tx, rx, reserved1, ack_requested, response_requested)
		...
		reserved1 = struct.unpack("Q", payload_str[8:16])[0]
		version = struct.unpack("I", payload_str[16:20])[0]
		message = StateWifiFirmware(target_addr, source_id, seq_num, build, reserved1, version, ack_requested, response_requested)
		...
		message = SetPower(target_addr, source_id, seq_num, power_level, ack_requested, response_requested)

	elif message_type == MSG_IDS[StatePower]:
		power_level = struct.unpack("H", payload_str[0:2])[0]
		message = StatePower(target_addr, source_id, seq_num, power_level, ack_requested, response_requested)
		...
		label = "".join([chr(b) for b in struct.unpack("b"*32, payload_str[0:32])])
		message = SetLabel(target_addr, source_id, seq_num, label, ack_requested, response_requested)

	elif message_type == MSG_IDS[StateLabel]:
		label = "".join([chr(b) for b in struct.unpack("b"*32, payload_str[0:32])])
		message = StateLabel(target_addr, source_id, seq_num, label, ack_requested, response_requested)
		...
		message = StateVersion(target_addr, source_id, seq_num, vendor, product, version, ack_requested, response_requested)
		...
		message = StateInfo(target_addr, source_id, seq_num, time, uptime, downtime, ack_requested, response_requested)
		...
		message = EchoRequest(target_addr, source_id, seq_num, byte_array, ack_requested, response_requested)

	elif message_type == MSG_IDS[EchoResponse]:
		byte_array_len = len(payload_str)
		byte_array = [b for b in struct.unpack("B"*byte_array_len, payload_str[0:byte_array_len])]
		message = EchoResponse(target_addr, source_id, seq_num, byte_array, ack_requested, response_requested)
		...
		message = LightSetColor(target_addr, source_id, seq_num, color, duration, ack_requested, response_requested)
		...
		message = LightState(target_addr, source_id, seq_num, color, reserved1, power_level, label, reserved2, ack_requested, response_requested)
		...
		message = LightSetPower(target_addr, source_id, seq_num, power_level, duration, ack_requested, response_requested)

	elif message_type == MSG_IDS[LightStatePower]:
		power_level = struct.unpack("H", payload_str[0:2])[0]
		message = LightStatePower(target_addr, source_id, seq_num, power_level, ack_requested, response_requested)
