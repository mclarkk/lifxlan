...
class Message(object):
    def __init__(
        ...
        # Frame Address
        self.frame_addr_format = [
            "uint:64",
            "uint:48",
            "uint:6, uint:1, uint:1",
            "uint:8",
        ]
    ...
    def get_frame(self):
        ...
        flags = little_endian(
            bitstring.pack(
                flags_format, self.origin, self.tagged, self.addressable, self.protocol
            )
        )
    ...
    def get_frame_addr(self):
        ...
        mac_addr = little_endian(
            bitstring.pack(mac_addr_format, convert_MAC_to_int(self.target_addr))
        )
    ...
    def get_protocol_header(self):
        ...
        message_type = little_endian(
            bitstring.pack(message_type_format, self.message_type)
        )
