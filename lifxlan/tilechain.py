...
class TileChain(Light):
        ...
        if (self.tile_info == None) or (refresh_cache == True):
            ...
            for tile in response.tile_devices:
                t = Tile(
                    ...
                    tile["width"],
                    tile["height"],
                    tile["device_version_vendor"],
                    tile["device_version_product"],
                    tile["device_version_version"],
                    tile["firmware_build"],
    ...
    def get_tile_colors(self, start_index, tile_count=1, x=0, y=0, width=8):
        if (start_index < 0) or (start_index >= self.tile_count):
            raise InvalidParameterException(
                "{} is not a valid start_index for TileChain with {} tiles.".format(
                    start_index, self.tile_count
                )
            )
    ...
    def set_tilechain_colors(self, tilechain_colors, duration=0, rapid=False):
        ...
        for i in range(self.tile_count):
            t = Thread(
                target=self.set_tile_colors,
                args=((i, tilechain_colors[i], duration, 1, 0, 0, 8, rapid)),
            )
            threads.append(t)
            t.start()
        for t in threads:
    ...
    def project_matrix(self, hsvk_matrix, duration=0, rapid=False):
        ...
        if (matrix_x != canvas_x) or (matrix_y != canvas_y):
            raise InvalidParameterException(
                "Warning: TileChain canvas wants a {} x {} matrix, but given matrix is {} x {}.".format(
                    canvas_x, canvas_y, matrix_x, matrix_y
                )
            )
        ...
        tile_colors = [
            [default_color for i in range(tile_width * tile_height)]
        ...
        for i, tile_color in enumerate(tile_colors):
            t = Thread(
                target=self.set_tile_colors,
                args=((i, tile_color, duration, 1, 0, 0, 8, rapid)),
            )
            threads.append(t)
            t.start()
        for t in threads:
    ...
    def set_tile_coordinates(self, tile_index, x, y):
        self.req_with_ack(
            SetUserPosition,
            {"tile_index": tile_index, "reserved": 0, "user_x": x, "user_y": y},
        )
    ...
    def get_canvas_dimensions(self, refresh_cache=False):
        if (self.canvas_dimensions == None) or (refresh_cache == True):
            ...
            tile_width = (
                8  # TO DO: get these programmatically for each light from the tile info
            )
            tile_height = (
                8  # TO DO: get these programmatically for each light from the tile info
            )
    ...
    def set_tile_effect(
        ...
        if len(palette) > 16:
            raise InvalidParameterException(
                "Maximum palette size is 16, {} given.".format(len(palette))
            )
        if len(parameters) > 8:
            raise InvalidParameterException(
                "Maximum parameters size is 8, {} given.".format(len(parameters))
            )
...
class Tile(object):
    def __init__(
        ...
        width=8,
        height=8,
        device_version_vendor=None,
        device_version_product=None,
        device_version_version=None,
        firmware_build=None,
