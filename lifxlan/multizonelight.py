...
class MultiZoneLight(Light):
    def __init__(
        ...
        super(MultiZoneLight, self).__init__(
    ...
    def get_color_zones(self, start=None, end=None):
        ...
        if start != None and end != None:
            ...
            if start >= total_zones:
                raise ValueError(
                    "In the function get_color_zones, starting index is greater than the total available zones (provided start = {}, end = {} for a device with {} total zones).".format(
                        start, end, total_zones
                    )
                )
            if end <= start:
                raise ValueError(
                    "In the function get_color_zones, end must be greater than start (provided start = {}, end = {}).".format(
                        start, end, total_zones
                    )
                )
        if (start != None and end == None) or (start == None and end != None):
            raise ValueError(
                "In the function get_color_zones, start and end indices must both be provided, or neither provided."
            )
        ...
        for i in range(int(math.ceil(total_zones / 8.0))):
            response = self.req_with_resp(
                MultiZoneGetColorZones,
                [MultiZoneStateZone, MultiZoneStateMultiZone],
                {"start_index": 0 + (i * 8), "end_index": 7 + (i * 8)},
            )
            ...
            if first_included_zone + 8 > total_zones:
                ...
                last_index = 8
            all_zones[first_included_zone : last_included_zone + 1] = response.color[
                : last_index + 1
            ]
    ...
    def set_zone_color(
        self, start_index, end_index, color, duration=0, rapid=False, apply=1
    ):
        if len(color) == 4:
            try:
                if rapid:
