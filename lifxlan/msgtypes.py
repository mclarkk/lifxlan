...
class StateHostInfo(Message):
    ...
    def get_payload(self):
        ...
        signal = little_endian(bitstring.pack("uint:32", self.signal))
        tx = little_endian(bitstring.pack("uint:32", self.tx))
        rx = little_endian(bitstring.pack("uint:32", self.rx))
        reserved1 = little_endian(bitstring.pack("uint:16", self.reserved1))
...
class SetLabel(Message):
    ...
    def get_payload(self):
        ...
        try:
            label = b"".join(
                little_endian(bitstring.pack("uint:8", c))
                for c in self.label.encode("utf-8")
            )
        except ValueError:  # because of differences in Python 2 and 3
            label = b"".join(
                little_endian(bitstring.pack("uint:8", ord(c)))
                for c in self.label.encode("utf-8")
            )
        padding = b"".join(
...
class LightSetWaveform(Message):
    ...
    def get_payload(self):
        ...
        payload = (
            reserved_8 + transient + color + period + cycles + duty_cycle + waveform
        )
        return payload
...
class GetMultiZoneEffect(Message):
    def __init__(
        ...
        super(GetMultiZoneEffect, self).__init__(
            MSG_IDS[GetMultiZoneEffect],
            target_addr,
            source_id,
            seq_num,
            ack_requested,
...
class SetMultiZoneEffect(Message):
    def __init__(
        ...
        super(SetMultiZoneEffect, self).__init__(
            MSG_IDS[SetMultiZoneEffect],
            target_addr,
            source_id,
            seq_num,
            ack_requested,
    ...
    def get_payload(self):
        ...
        reserved2 = little_endian(bitstring.pack("uint:32", self.reserved2))
        reserved3 = little_endian(bitstring.pack("uint:32", self.reserved3))
        payload = (
            instanceid
            + effect_type
            + reserved1
...
class StateMultiZoneEffect(Message):
    def __init__(
        ...
        self.reserved3 = payload["reserved3"]
        self.parameters = payload["parameters"]
        super(StateMultiZoneEffect, self).__init__(
            MSG_IDS[StateMultiZoneEffect],
            target_addr,
            source_id,
            seq_num,
            ack_requested,
...
class GetDeviceChain(Message):
    def __init__(
        ...
        target_addr = BROADCAST_MAC
        super(GetDeviceChain, self).__init__(
            MSG_IDS[GetDeviceChain],
            target_addr,
            source_id,
            seq_num,
            ack_requested,
...
class StateDeviceChain(Message):
    def __init__(
        ...
        super(StateDeviceChain, self).__init__(
            MSG_IDS[StateDeviceChain],
            target_addr,
            source_id,
            seq_num,
            ack_requested,
    ...
    def get_payload(self):
        ...
        for tile in self.tile_devices:
            ...
            device_version_vendor = little_endian(
                bitstring.pack("uint:32", tile["device_version_vendor"])
            )
            device_version_product = little_endian(
                bitstring.pack("uint:32", tile["device_version_product"])
            )
            device_version_version = little_endian(
                bitstring.pack("uint:32", tile["device_version_version"])
            )
            firmware_build = little_endian(
                bitstring.pack("uint:64", tile["firmware_build"])
            )
            reserved6 = little_endian(bitstring.pack("uint:64", tile["reserved6"]))
            firmware_version = little_endian(
                bitstring.pack("uint:32", tile["firmware_version"])
            )
...
class SetUserPosition(Message):
    def __init__(
        ...
        super(SetUserPosition, self).__init__(
            MSG_IDS[SetUserPosition],
            target_addr,
            source_id,
            seq_num,
            ack_requested,
...
class GetTileState64(Message):
    def __init__(
        ...
        super(GetTileState64, self).__init__(
            MSG_IDS[GetTileState64],
            target_addr,
            source_id,
            seq_num,
            ack_requested,
...
class StateTileState64(Message):
    def __init__(
        ...
        super(StateTileState64, self).__init__(
            MSG_IDS[StateTileState64],
            target_addr,
            source_id,
            seq_num,
            ack_requested,
...
class SetTileState64(Message):
    def __init__(
        ...
        self.duration = payload["duration"]
        self.colors = payload["colors"]
        super(SetTileState64, self).__init__(
            MSG_IDS[SetTileState64],
            target_addr,
            source_id,
            seq_num,
            ack_requested,
...
class GetTileEffect(Message):
    def __init__(
        ...
        super(GetTileEffect, self).__init__(
            MSG_IDS[GetTileEffect],
            target_addr,
            source_id,
            seq_num,
            ack_requested,
...
class SetTileEffect(Message):
    def __init__(
        ...
        super(SetTileEffect, self).__init__(
            MSG_IDS[SetTileEffect],
            target_addr,
            source_id,
            seq_num,
            ack_requested,
    ...
    def get_payload(self):
        ...
        duration = little_endian(bitstring.pack("uint:64", self.duration))
        reserved3 = little_endian(bitstring.pack("uint:32", self.reserved3))
        reserved4 = little_endian(bitstring.pack("uint:32", self.reserved4))
        payload = (
            reserved1
            + reserved2
            + instanceid
...
class StateTileEffect(Message):
    def __init__(
        ...
        self.palette_count = payload["palette_count"]
        self.palette = payload["palette"]
        super(StateTileEffect, self).__init__(
            MSG_IDS[StateTileEffect],
            target_addr,
            source_id,
            seq_num,
            ack_requested,
...
class ButtonState(Message):
    def __init__(
        ...
        super(ButtonState, self).__init__(
            MSG_IDS[ButtonState],
            target_addr,
            source_id,
            seq_num,
    ...
    def get_payload(self):
        ...
        buttons_count = little_endian(bitstring.pack("uint:8", self.buttons_count))
        payload = count + index + buttons_count
        for button in self.buttons:
            actions_count = little_endian(
                bitstring.pack("uint:8", button["actions_count"])
            )
            payload += actions_count
            for action in button["actions"]:
                gesture = little_endian(bitstring.pack("uint:16", action["gesture"]))
                target_type = little_endian(
                    bitstring.pack("uint:16", action["target_type"])
                )
